 ...
 * @property TblBookings[] $tblBookings
...
class Requests extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, car_id, company_id, system_id, subsystem_id, no_description, on_behalf, request_raised_user_id, email_sent, deleted_status, status', 'numerical', 'integerOnly'=>true),
			array('request_type', 'length', 'max'=>100),
			array('previous_millage, current_millage, fuel_quantity', 'length', 'max'=>50),
			array('description', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, car_id, company_id, request_type, system_id, subsystem_id, previous_millage, current_millage, fuel_quantity, description, no_description, on_behalf, request_raised_user_id, created_at, email_sent, deleted_status, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'Bookings' => array(self::HAS_MANY, 'Bookings', 'request_id'),
	...
	public function requestUser($first_name,$last_name)
	{
	 ...
	 public function RequestSubDetails($id,$no_description,$type)
	 {
		...
		if($no_description==0){
			...
			$RequestSubDetails=RequestSubDetails::model()->findAllByAttributes(array('request_id'=>$id)); 
	...
	public function SendEmail($email_sent,$id)
	{
	 ...
	 public function BookCar($id,$type)
	 { 
		$controller=Yii::app()->controller->createUrl('Bookings/create')."&rid=".base64_encode($id)."&type=".base64_encode($type);
		$exportLink="<a href='".$controller."'>Add Job Card</a>";
		//Book Car
		$Bookings=Bookings::model()->findByAttributes(array('request_id'=>$id));
		if(count($Bookings)==0){
			echo $exportLink;
		}else{
			$supplier=Suppliers::model()->findByPk($Bookings->supplier_id);
			echo "<b>Job Card Created</b> (".$supplier->title.")";
		//Booked
		}
	 }
	
	
	public function millage($previous_millage,$current_millage)
	{
		$millage=intval($current_millage-$previous_millage);
		echo "<b>".$millage."</b>";
	}
	
	
	public function CurrentFuelConsumption($previous_millage,$current_millage,$fuel_quantity,$consumption)
	{
		$fuelConsumption=round(($current_millage-$previous_millage)/$fuel_quantity,1);
		if($fuelConsumption<($consumption-5)){
			$text="<font color='#FF0000'><b>".$fuelConsumption."</b></font>";
		}else{
			$text="<font color='#000099'>".$fuelConsumption."</font>";
		}
		echo $text;
	}
	...
	public function search($request_type,$role,$user_id)
	{
		...
		$criteria->compare('fuel_quantity',$this->fuel_quantity,true);
