...
class RequestComments extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('request_id, user_id, supplier_id, actor, created_at', 'required'),
			array('request_id, user_id, supplier_id, no_photo, no_description, status', 'numerical', 'integerOnly'=>true),
			array('actor', 'length', 'max'=>50),
			array('photo', 'length', 'max'=>100),
			array('description', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, request_id, user_id, supplier_id, actor, no_photo, photo, description, no_description, created_at, status', 'safe', 'on'=>'search'),
		);
	}
