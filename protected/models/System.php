...
class System extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, service_type', 'required'),
			array('is_default, status', 'numerical', 'integerOnly'=>true),
			array('title, service_type', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, is_default, service_type, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'SupplierSystems' => array(self::HAS_MANY, 'SupplierSystems', 'system_id'),
		);
	}
