...
/**
 * This is the model class for table "tbl_spare".
 *
 * The followings are the available columns in table 'tbl_spare':
 * @property integer $id
...
class Spare extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('sub_system_id, status', 'required'),
			array('sub_system_id, status', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, sub_system_id, title, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'subSystem' => array(self::BELONGS_TO, 'SubSystem', 'sub_system_id'),
		);
	}
	...
	public function deleteRecord($id)
   {
	...
	public function search()
	{
		...
		$criteria->compare('sub_system_id',$this->sub_system_id);
