...
if($model->photo!=""){
	?>
    <?php echo CHtml::image(Yii::app()->request->baseUrl.'/car_mechanical_issues/'.$model->photo,"advert",array("width"=>100,"height"=>100)); ?> 
    <?php
}
?>
...
<?php echo CHtml::Button($model->isNewRecord ? 'Add' : 'Update',array('onClick'=>'PostMechanicalIssues()')); ?>
<br>
...
function PostMechanicalIssues()
{
	var current_task=$("#current_task").val();	
	var CarMechanicalIssues_car_id=$("#CarMechanicalIssues_car_id").val();
	...
	var CarMechanicalIssues_status=$("#CarMechanicalIssues_status").val();
	
	if(photo_hidden=="" && CarMechanicalIssues_photo=="")
	{
		var ext="";
	}else if(photo_hidden!="" && CarMechanicalIssues_photo=="")
	{
		var ext="jpg";
	}else if(photo_hidden=="" && CarMechanicalIssues_photo!="")
	{
		...
		if(ext=="png" || ext=="jpg" || ext=="jpeg"){
			...
			if(CarMechanicalIssues_description!==""){
				...
				if(CarMechanicalIssues_status!==""){
					...
					if(current_task=='Add'){
						$('#car-mechanical-issues-form')[0].reset();
