...
if($model->isNewRecord){
	$cars=Cars::model()->findAll("status='1' AND is_default=0 AND company_id=$LOGGED_IN_COMPANY AND is_assigned=0"); 
}else{
...
if($model->isNewRecord){
	$sql="SELECT * from tbl_users WHERE status='1' AND role_id=".$driver_role_id." AND company_id=$LOGGED_IN_COMPANY "."
...
echo CHtml::activeDropDownList($model, 'user_id', $data ,array('empty' => '--Select User (Driver)--','class'=>'parent_form_elements'));
?>
...
<?php echo CHtml::Button($model->isNewRecord ? 'Add' : 'Update',array('onClick'=>'PostCarsAssignment()','class'=>'parent_form_btn')); ?>
<br>
