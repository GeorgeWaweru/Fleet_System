...
{
	?>
    <b>Repair request for car- <?php echo $CarsData->number_plate;?></b><br><br>
    <?php
}
?>
<table>   
...
<td>
 <?php echo CHtml::activeDropDownList($model,'system_id',CHtml::listData(System::model()->findAll("status='1' AND service_type='Repair' AND is_default=0"),'id','title') ,array('empty' => '--Select System--','onchange'=>'getSubSystem(this)','class'=>'parent_fields parent_form_elements')); ?>
...
<td>
<?php echo $form->textArea($model,'description',array('rows'=>6, 'cols'=>50,'class'=>'parent_fields','class'=>'parent_form_elements')); ?>
<br>
<b>No Description</b>
...
<?php echo $form->hiddenField($model,'request_type',array('size'=>60,'maxlength'=>200,'value'=>'repair')); ?>
<?php
if($COMPANY_SUB_USER_ROLE=='Driver')
{
...
<?php echo CHtml::Button($model->isNewRecord ? 'Add' : 'Update',array('onClick'=>'PostRepair()','class'=>'parent_form_btn')); ?>
<br>
...
<div id="info_frm"></div>
	...
	?>
    <div class="error">You do not have a car assigned to you at the moment thus you cannot raise a repair request</div>
    <?php
}
?>
<?php $this->endWidget(); ?>
...
$("#sub_details_preloading_div").hide();
	if(parseInt(response)==1){
		...
		$("#sub_details_info_div").addClass('success').removeClass('error').html('Repair Request details successfully saved.').show();
		getRequestSubDetails(RequestSubDetails_request_id);
	}else{
		$("#sub_details_info_div").addClass('error').removeClass('success').html('Error Repair Request details.').show();
	}	
}
});
/*End submit the form*/
		...
		if(Requests_subsystem_id>0){
			...
			if((Requests_description!=="") ||  (Requests_description=="" && Requests_no_description==true)){
				...
				if(Requests_status>0){
					...
					$("#info_frm").addClass('success').removeClass('error').html('Repair Request successfully added.').show();
					}else if(current_task=='Update'){
					$("#info_frm").addClass('success').removeClass('error').html('Repair Request successfully updated.').show();
					}
					}
					}
					}).submit();
