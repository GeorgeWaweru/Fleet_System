...
<?php echo CHtml::Button($model->isNewRecord ? 'Add' : 'Update',array('onClick'=>'PostEngines()','class'=>'parent_form_btn')); ?>
<br>
...
function PostEngines()
{
	var current_task=$("#current_task").val();
	var Engines_title=$("#Engines_title").val();
	var Engines_status=$("#Engines_status").val();
	if(Engines_title!==""){
		...
		if(Engines_status!==""){
			...
			if(current_task=='Add'){
			$('#engines-form')[0].reset();
			$("#info_frm").addClass('success').removeClass('error').html('Engine Number successfully added.').show();
			}else if(current_task=='Update'){
			$("#info_frm").addClass('success').removeClass('error').html('Engine Number successfully updated.').show();
			}
			}
			}
			}).submit();
		...
		displayError("Please enter Engine Number.","title_error");
	}
}
