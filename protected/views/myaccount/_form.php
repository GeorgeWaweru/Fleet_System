...
function ResetPassword()
{
	...
	var prog_bar=getCookie('PASSWORD_STRNGTH');
	...
	if(user_role_level=="Low"){
		...
		if(password!=""){
			...
			if(prog_bar== "Strong" || prog_bar== "Excellent"){
				$("#password_error").html("").hide();
				...
				if(confirm_password!=""){
						...
						beforeSend: function(){
							...
							$("#password").val('');
							$("#confirm_password").val('');
							setCookie("PASSWORD_STRNGTH","Blank");
						});		
	
	
				}else{
						$("#password_error").html("password do not match").show();
						$("#confirm_password_error").html("password do not match").show();
				}
						
				}else{
						$("#confirm_password_error").html("Confirm password cannot be blank").show();
				}
					
			}else{
				$("#password_error").html("Your password is not strong enough").show();
			}
		}else{
			$("#password_error").html("password cannot be blank").show();
		}
...
<script language="javascript" type="text/javascript">
	$(document).keypress(function(e) {
		if(e.which == 13) {
				document.getElementById("systemusers-form").submit();
		}
	});
</script>
